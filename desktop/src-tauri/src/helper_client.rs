...
pub fn send_domain_list(active_domains: &[String]) -> Result<HelperResponse, String> {
    let name = HELPER_SOCKET_NAME.to_ns_name::<GenericNamespaced>().map_err(|e|
        format!("invalid helper socket name: {e}")
    )?;

    let mut conn = Stream::connect(name).map_err(|e| {
        format!("failed to connect to the privileged helper (is it installed and running?): {e}")
    })?;

    let request = HelperRequest {
        active_domains: active_domains.to_vec(),
    };
    let mut request_json = serde_json
        ::to_string(&request)
    ...
    reader.read_line(&mut line).map_err(|e| format!("failed to read helper response: {e}"))?;
